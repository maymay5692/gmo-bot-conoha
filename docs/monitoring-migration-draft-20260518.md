...
status: **v0.2 finalize** (2026-05-22 中間レビュー B5 mentor 確定 = 試験並走 5/22→6/1 (10 日間) 採用、本書 session 21 = 2026-05-23 で finalize 化。過去履歴: 2026-05-18 v0.1 初稿、2026-05-19 session 17 で conoha 側 position paper 追加、2026-05-22 中間レビューで B5 確定、6/1 本移管判定予定)
...
## ★ 2026-05-22 中間レビュー B5 mentor 確定 (5 論点)

mentor + scout + conoha の 3 者擦り合わせ完了、以下確定:

| # | 論点 | mentor 確定内容 |
|---|---|---|
| 1 | データ受け渡し方法 | **push 方式** (scout → 共有 path `~/Desktop/hl-monitoring-shared/`)、conoha は独立タイミングで読みに行く |
| 2 | アナウンス検出時の即日通知経路 | **file watch + mentor 経由 二重化** (即時性 + 人間チェーン fallback) |
| 3 | データ source 一致確認 | **monitoring file に scout source 併記**、HYPE 価格 ±2% 以内 / アナウンス検出有無一致を 5/22-6/1 で確認 |
| 4 | scout 障害時 fallback | **24h 検知ルール**: scout 毎週月曜 12:00 JST 「採取完了 ping」がなければ 24h 後に conoha 一時代替 |
| 5 | 試験並走期間 | **5/22 → 6/1 (10 日間)** 採用 (conoha 提案 14 日間 = 5/22-6/5 から短縮、scout 負荷考慮)、6/1 で本移管判定 |

## ★ conoha 側 position paper (session 17 = 2026-05-19 追加、5/22 mentor 確定後の位置付け)

5/22 中間レビューで上記 5 論点を効率化するため、conoha 側の position を事前に明文化。**5/22 mentor 確定で大半採用、ただし論点 5 (試験並走期間) のみ修正 (conoha 提案 14 日間 → mentor 確定 10 日間)**。以下原文を保持、各論点末尾に「mentor 確定」マーカーを追記。
...
→ **★ mentor 5/22 B5 確定: 採用** (`~/Desktop/hl-monitoring-shared/` push 方式)
...
→ **★ mentor 5/22 B5 確定: 採用** (file watch + mentor 経由 二重化)
...
→ **★ mentor 5/22 B5 確定: 採用** (monitoring file に scout source 併記、HYPE 価格 ±2% 以内 / アナウンス検出一致を 5/22-6/1 で確認)
...
→ **★ mentor 5/22 B5 確定: 採用** (24h 検知ルール + conoha 一時代替)

### 論点 5: 5/22 → 6/1 試験移管 — conoha 提案 14 日間 → ★ mentor 確定 10 日間

**conoha 提案 (5/19 session 17)**: mentor 当初提示の 6/1 から 4 日延長して **5/22-6/5 (2 週間並走、14 日間)**。
...
→ **★ mentor 5/22 B5 確定: 5/22 → 6/1 (10 日間) 採用** (conoha 提案 14 日間から 4 日短縮、scout 負荷考慮 + W22 (5/25 採取) + 5/26-31 の追加 1 サンプルで 2 サンプル取得確保のため)

**並走判定基準 (mentor 5/22 確定)**:
- 10 日間 (5/22-6/1) で HYPE 価格 ±2% 以内、HL 公式追跡の重大投稿 catchup 漏れゼロ、アナウンス検出有無一致
- 上記 OK なら **6/1 で本移管判定** (mentor 承認で scout 単独採取に移行)
- 不整合あれば mentor 判断で追加並走 (期間は B5 反映後に再判定)

## 移管完了の判定基準 (★ mentor 5/22 B5 確定)

- 試験並走期間 (5/22-6/1、10 日間) で scout 採取と conoha 採取値の整合 (HYPE 価格 ±2% 以内 + アナウンス検出有無一致 + 重大投稿 catchup 漏れゼロ)
- scout 障害時の fallback 経路 (24h 検知 + conoha 一時代替) が機能
- Trigger 検出時の通知経路 (file watch + mentor 経由 二重化) が機能
- → **6/1 mentor 承認で本移管判定**、以降 scout 単独採取に移行
- 不整合あれば mentor 判断で追加並走 (期間は判定時に再決定)
