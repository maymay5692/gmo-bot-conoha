...
**DRAFT v8 (2026-06-16, mentor 6/15 月次レビュー fed-back 反映)** — mentor 6/15 月次レビュー裁定 (`~/Desktop/my mentor/prompts/2026-06-16-conoha-monthly-review-7items.md`) で承認された 3 候補を反映。**#8 HF cluster trigger 閾値再校正** (active-only HF% → 安定分母 HF_core_share、Active 数正規化)、**#9 Gate 3 要件 5 事後評価ループ補強** (log フォーマット template + prior 更新ルール + 配布後プロセス annotation)、**#10 Gate 3 要件 2/3/4 軽微補強 bundle** (比較可能性に validator/insurance 軸追加 / 中サブセット閾値 / Gate 2↔Gate 3 発火順序)。**本 v8 は中間 (第 2 弾アナウンス前) の誤発火修正 + Gate 3 補強であり、配布細則発表後の期待値再計算 finalize は後続バンプ (v9 / v8-final) に送る** (下記「v7 残タスク」項目 6 参照)。

### v8 の更新内容 (mentor 6/15 月次レビュー fed-back、2026-06-16 session 34)

- **#8 HF cluster trigger 閾値再校正 (案B Active 数正規化、承認)**: 旧 active-only HF% が active validator 数の振動で誤発火する構造を 2026-06-14 interim で決定的に実証 (Active 24→27 復帰で 54.92%→49.09%、S_HF ~213M 不変)。trigger 指標を **安定分母 `HF_core_share = S_HF / D_nonjailed`** に変更 (Gate 2-7 新設)。閾値 55%/60% は mentor 承認どおり維持し安定分母上で評価。正規化式の内部設計は conoha 領分 (mentor 裁定)。検証: `scripts/data_cache/hl_hf_cluster_recalibration_20260616.md`
- **#9 Gate 3 要件 5 事後評価ループ補強 (最終承認)**: mentor 5/29 で 3 アクション全採用◯、6/15 最終承認。(1) 事後 validation log フォーマット template (比較可能性 4 軸 × 8 事例 + 「当たり/外れ/判定不能」3 値)、(2) prior 更新ルール明文化 (単純頻度更新を default、ベイズは将来オプション)、(3) HL2 配布完了直後の事後評価プロセス annotation (retro bump トリガ + 14 日期限) を Gate 3 要件 5 に確定。出典: `docs/conoha-5-requirements-coverage-20260529.md` §要件5
- **#10 Gate 3 要件 2/3/4 軽微補強 bundle (採用承認・低優先)**: 要件 2 比較可能性評点表に validator concentration 軸 + insurance fund 軸を追加、要件 3 に中サブセット閾値、要件 4 に Gate 2 trigger ↔ Gate 3 反証の発火順序ルールを明文化。維持に必要な補強のみ (機能拡張は追わない、mentor 降格領域方針)
- **裁定で保留・見送りになった項目 (本 spec では変更なし、status に記録)**: HEIKIN bot Mac→VPS 移行 (verdict(3) 保留 + strategy-lab 維持モード降格) / sho-gun note cron 移行 (6/28 判定待ち) / ConoHa plan upgrade (殿カテゴリ1、mentor 推奨=見送り、殿判断待ち) / scout monitoring 移管 retry (park、月次再評価)
...
- **Weekly monitoring 統合 (v4 起動、v7 拡張)**: 以下を週次で手動確認し `scripts/data_cache/hl_monitoring_YYYYww.md` に記録。**初回テンプレ `hl_monitoring_2026w17.md` 起票完了** (8 セクション: アナウンス監視 / Insurance fund / bug bounty / TVL Top 3 / HYPE 価格配分判定 / ToS 差分 / Validator 分散 / touch 行動ログ)
  ...
  - **HF cluster 合算 watch (session 4 追加、v7 で formal 化、v8 #8 で指標再校正)**: 2026-04-23 W17 baseline で Hyper Foundation 運用 nodes (HF 1-5) cluster 合算 **54.41%** を確認。個別 trigger は OK だが operator 単位では Foundation 支配的。**v8 #8 で trigger 指標を「active-only HF%」から「安定分母 HF_core_share」に再校正** (下記 2-7)。trigger 条件は **HF_core_share > 55% が 2 週連続 or HF 単体 60% 超** で spec 再評価セッション開始 (Gate 2 Tail Safety + Gate 2-6 レジーム変更の共同 trigger)

#### 2-7. HF cluster trigger 閾値再校正 (案B Active 数正規化, v8 #8)

出典: mentor 6/15 月次レビュー裁定 (`~/Desktop/my mentor/prompts/2026-06-16-conoha-monthly-review-7items.md` #8 承認、正規化式の内部設計は conoha 領分)。検証 raw: `scripts/data_cache/hl_hf_cluster_recalibration_20260616.md`。

**背景 — 旧指標 (active-only HF%) の誤発火構造 (実証済)**:
- 旧指標 `HF% = S_HF / D_active` は分母 `D_active` (active validator 総 stake) が振動する。marginal な non-jailed validator が active⇄inactive を toggle すると、`S_HF` (HF 5 nodes 合算 stake) 不変でも HF% が機械的に上下する。
- 振動サイクル一巡が実測された: W22 Active27 → 54.07% / W23-W24 Active24 → **55.0% 超 (旧 trigger 2 週連続誤発火)** / W25 Active24 → 54.95% / 6/14・2026-06-16 Active27 復帰 → **49.1%**。`S_HF` は全期間 ~213-214M で構造的に安定 (HF は accumulation していない)。同一 3 validator が toggle、合算 ~46.5M (6/14 再活性化 delta で確定)。
- これらの toggle validator は **jailed ではなく inactive-not-jailed として `validatorSummaries` に stake 込みで残存** (Total count に計上、`isJailed=false` & `isActive=false`)。

**再校正後の指標 (案B = 安定分母 share)**:
- **Primary**: `HF_core_share = S_HF / D_nonjailed`、`D_nonjailed = Σ stake (isJailed=false)` = active + inactive-not-jailed。toggle validator を分母に保持し active 数振動を吸収する (= 「Active 数正規化」の数理的実体)。
- **Co-indicator (毎週併記)**: `S_HF` 絶対 stake (HYPE) / Active・inactive-not-jailed・jailed counts / `D_active` / `D_nonjailed` / `HF_core_share` / (参考) 旧 active-only HF%。`S_HF` 絶対が accumulation の sanity check。
- **Trigger**: `HF_core_share > 55% が 2 週連続` または `HF 単体ノード > 60%` で spec 再評価セッション開始。閾値 55%/60% は mentor 承認どおり維持し、**安定分母上で評価** (2026-06-16 live で `HF_core_share = 49.16%`、~6pt headroom)。
- **誤発火ガード (fallback)**: inactive stake が取得不能で active-only% しか出せない週は Active count を必ず併記。active-only% > 55% でも (a) Active < 基準 (=27) かつ (b) `S_HF` 前週比横ばい (±2%) なら「分母振動由来の見かけ超過」と判定し、再評価を起動しない。

**遡及検算 (新指標なら誤発火しない実証)**: W23-W24 の 3 inactive-not-jailed (~46.5M) を分母に保持すれば `HF_core_share ≈ 213.3M / 434.5M ≈ 49%` < 55% → 旧 trigger の誤発火は新指標では起こらない。session 28 の手動 counterfactual 推定 (54.14%) は実際 (49.09%) から **5pt 外れ**ており、inactive stake を毎週 API 実測し安定分母で計算する方式 (本指標) が手動推定の eyeball 誤差を排除する。

**却下した代替**: count 正規化 (stake が validator 間で極端に不均等なため不正確) / 絶対 stake 単独 trigger (share の本質を見失う、co-indicator に留める) / 閾値 55%→57% 引き上げ (W25 案 A、root cause を放置)。
...
**v8 #10 補強 (比較可能性評点軸の追加)**: 上記 4 軸 (プロトコル規模 / 参加形態類似 / 競合密度類似 / 総合) に加え、**validator concentration 軸** (Top 1/3/5 share, HF-equivalent cluster ratio) と **insurance fund 軸** (規模 / stablecoin diversification) を評点列として追加する。HL1 列には retro v0.5 §12 Tail Safety 10 項目のデータ (清算バッファ / OI 上限 / 中央集権度) を反映。優先度: 低 (mentor 裁定、維持に必要な補強のみ、機能拡張は追わない)。

### 要件 3. 層別閾値 (spec 事前宣言)

- **高サブセット (HL1, EIGEN)**: **100% ポジティブ必須** (2/2)。1 件でもネガなら HL2 はイベント特殊性疑いで要再検討
- **中サブセット ({JTO, JUP, APT, ARB, DYDX, UNI}, 6 件)** (v8 #10 追加): **50% 以上ポジティブ** (3/6 以上)。比較可能性中位群のため高サブセットより緩い中間閾値 (wiki §8「比較可能性低サブセットでは緩め」に対応)
...
**v8 #10 補強 (Gate 2 monitoring trigger ↔ Gate 3 反証の発火順序)**: 配布完了前に Gate 2 trigger (HF cluster / Insurance / Validator 等) と Gate 3 反証が同時期に発火した場合の優先順位を明文化する:
- **Gate 2 trigger 先行発火** → 配分縮小 (B → C 方向) + Gate 3 反証データ収集を継続 (戦略は保留せず monitoring 強化)
- **Gate 3 反証先行確定** → 戦略保留 / 破棄判定、Gate 2 monitoring は継続するが新規 touch を停止
- **両者同時発火** → 戦略破棄を優先

### 要件 5. 事後評価ループ (v8 #9 で補強確定)

mentor 6/15 月次レビューで最終承認 (5/29 で 3 アクション全採用◯)。出典: `docs/conoha-5-requirements-coverage-20260529.md` §要件5。

**(1) 事後 validation log フォーマット template** — HL2 配布完了後、比較可能性評点 (8 事例 × 4 軸) の予測 vs 実測を下記テーブルに記録する。各事例の判定は「当たり / 外れ / 判定不能」の 3 値:

| 事例 | 事前評点 (総合) | 事前予測 (ポジ/ネガ) | 実測結果 | 判定 (当たり/外れ/判定不能) | 備考 |
|---|---|---|---|---|---|
| HL1 | 高 | ポジ | (配布後記入) | | 高サブセット |
| EIGEN | 高 | ポジ | | | 高サブセット |
| JTO / JUP / APT / ARB / DYDX | 中 | (各記入) | | | 中サブセット |
| UNI | 中 | ネガ寄り | | | |

- 「判定不能」= 配布構造が想定と質的に異なり予測の当否を問えないケース (記録のみ、prior 更新の分母から除外)。

**(2) prior 更新ルール (明文化)** — default は **単純頻度更新**: 次回エアドロ戦略 spec の prior = (当たり件数) / (当たり + 外れ件数) ※判定不能は分母から除外。ベイズ更新 (Beta 事前分布) は将来オプションとして保持するが、初版は単純頻度で運用 (over-engineering 回避)。

**(3) HL2 配布完了直後の事後評価プロセス (annotation)**:
- トリガ = HL2 配布完了 (公式 claim 開始) の検出。
- 期限 = 配布完了から **14 日以内**に retro を bump (現 `docs/hl-airdrop-s1-retro.md` v0.5 → v0.6 or v1.0) し、本要件 5 の log テーブルを記入。
- prior 更新値を次回エアドロ戦略 spec の Gate 1 N_trials / 事前確率に反映。
- 記録先は retro 文書 (現行のバージョン管理規律を継続)。当初の仮称 `analyses/hyperliquid-airdrop-retro-2026.md` は新規作成せず、既存 retro に統合する。
...
### v8 完了項目 (2026-06-16 session 34、mentor 6/15 月次レビュー fed-back)

- [x] **#8 HF cluster trigger 閾値再校正** (案B Active 数正規化、mentor 承認): Gate 2-7 新設、trigger 指標を active-only HF% → 安定分母 `HF_core_share = S_HF / D_nonjailed` に変更。閾値 55%/60% は維持し安定分母上で評価。6/14 interim + 2026-06-16 live (49.16%) で誤発火構造を実証。検証: `scripts/data_cache/hl_hf_cluster_recalibration_20260616.md`
- [x] **#9 Gate 3 要件 5 事後評価ループ補強** (mentor 最終承認): log フォーマット template (8 事例 × 4 軸 + 3 値判定) + prior 更新ルール (単純頻度更新 default) + 配布後 14 日以内 retro bump プロセスを要件 5 に確定
- [x] **#10 Gate 3 要件 2/3/4 軽微補強 bundle** (mentor 採用承認・低優先): 要件 2 に validator/insurance 評点軸、要件 3 に中サブセット閾値 (3/6)、要件 4 に Gate 2↔Gate 3 発火順序ルールを追加
- [—] **保留・見送り (本 spec 変更なし、status 記録)**: HEIKIN bot / sho-gun note cron の VPS 移行、ConoHa plan upgrade (殿判断待ち)、scout 移管 retry (park)
...
6. **第 2 弾アナウンス後の v8 → v9 (v8-final) 再 finalize**: HL 公式細則発表後、期待値 20% 年率 / 配布条件 / snapshot date を実数に置換して期待値表を再計算 (本 v8 は中間バンプ = 誤発火修正 #8 + Gate 3 補強 #9/#10。配布細則ベースの期待値 finalize は外部トリガ待ちで後続バンプに送る)
